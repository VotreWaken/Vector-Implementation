...
private:
	...
	// SimpleCopy - Если есть возможность без исключений переместить вектор избежав копирование
	// В противном случае копирует 


	// Перемещает, когда исключений не гарантируется,
	// в противном случае копирует.
	template<typename U>
	...
	// Класс признаков, который определяет, является ли T тривиально разрушаемым типом.
	// К тривиально разрушаемым типам относятся скалярные типы , тривиально копирующие конструируемые классы 
	// и массивы таких типов.
	// 
	// Тривиально разрушаемый класс — это класс(определенный с помощью class, struct или union), который:
	// использует неявно определенный деструктор.



	// Когда у элементов нет своего деструктора вручную с помощью цикла их удаляем
	template<typename U>
	...
	// Избегайте выделения ресурсов, когда исключений не гарантировано.
	// т.е. При копировании целых чисел повторно используйте буфер,
	// если это возможно, чтобы избежать дорогостоящего выделения
	// ресурсов.
	template<typename U>
